...
Build WaffleHouse-Client 5.4 alpha2 for Linux or FreeBSD (GUI + CLI).
...
echo
echo "==> Configuring WaffleHouse-Client 5.4 alpha2"
...
echo
echo "==> Building WaffleHouse-Client 5.4 alpha2"
...
if [ "$INSTALL_MODE" = yes ]; then
  echo
  echo "==> Installing WaffleHouse-Client 5.4 alpha2"
  prepare_privileges
  if [ -e "$INSTALL_BIN" ] || [ -L "$INSTALL_BIN" ]; then
...
if [ "$DRY_RUN" -eq 1 ]; then echo "Dry run complete."; else echo "WaffleHouse-Client 5.4 alpha2 build complete."; fi
