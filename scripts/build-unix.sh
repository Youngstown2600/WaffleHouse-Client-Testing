...
Build WaffleHouse-Client 5.6 for Linux or FreeBSD (GUI + CLI).
...
echo
echo "==> Configuring WaffleHouse-Client 5.6"
...
echo
echo "==> Building WaffleHouse-Client 5.6"
...
if [ "$INSTALL_MODE" = yes ]; then
  echo
  echo "==> Installing WaffleHouse-Client 5.6"
  prepare_privileges
  if [ -e "$INSTALL_BIN" ] || [ -L "$INSTALL_BIN" ]; then
...
if [ "$DRY_RUN" -eq 1 ]; then echo "Dry run complete."; else echo "WaffleHouse-Client 5.6 build complete."; fi
